...
app.get("/", async (c) => {
	...
	return c.html(layout(content, "Enhanced Memory MCP Server - Home"));
});
...
app.get("/authorize", async (c) => {
	...
	const oauthScopes = [
		{
			name: "read_memories",
			description: "Read your stored memories and search through them",
		},
		{ name: "write_memories", description: "Create and modify your memories" },
		{ name: "manage_projects", description: "Switch between memory projects and view analytics" },
	];

	if (isLoggedIn) {
		const content = await renderLoggedInAuthorizeScreen(oauthScopes, oauthReqInfo);
		return c.html(layout(content, "Enhanced Memory MCP Server - Authorization"));
	}
		...
		if (false) {
			return c.html(
				layout(
					...
					"Enhanced Memory MCP Server - Authorization Status",
				),
			);
		}
	}
